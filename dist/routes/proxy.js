 ...
 * /v1/{path}:
 *   get:
 *     summary: Proxy GET a la API maestra
 *     description: Reenvía una solicitud GET a la API maestra
 *     tags: [Proxy]
 *     parameters:
 *       - in: path
 *         name: path
 ...
 *         schema:
 *           type: string
 *         description: Ruta a la API maestra
 *     security:
 *       - ApiKeyAuth: []
 *     responses:
 *       '200':
 *         description: Respuesta exitosa de la API maestra
 *       '401':
 *         description: No autorizado
 *         content:
 *           application/json:
 ...
 *               $ref: '#/components/schemas/Error'
 *   post:
 *     summary: Proxy POST a la API maestra
 *     description: Reenvía una solicitud POST a la API maestra
 *     tags: [Proxy]
 *     parameters:
 *       - in: path
 *         name: path
 ...
 *         description: Error al comunicarse con la API maestra
 *   put:
 *     summary: Proxy PUT a la API maestra
 *     description: Reenvía una solicitud PUT a la API maestra
 *     tags: [Proxy]
 *     parameters:
 *       - in: path
 *         name: path
 ...
 *         description: Error al comunicarse con la API maestra
 *   delete:
 *     summary: Proxy DELETE a la API maestra
 *     description: Reenvía una solicitud DELETE a la API maestra
 *     tags: [Proxy]
 *     parameters:
 *       - in: path
 *         name: path
 ...
 *         description: Error al comunicarse con la API maestra
 */
// Manejar todas las solicitudes y métodos
router.all('/*', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Construir la URL de destino relativo al mount actual (/v1)
        const relativeUrl = req.originalUrl.slice((req.baseUrl || '').length) || '/';
        // Reescritura: /api/session -> /api/sessions
        const [pathOnly, queryPart] = relativeUrl.split('?');
        const rewrittenPath = pathOnly === '/api/session' ? '/api/sessions' : pathOnly;
        const targetUrl = `${config_1.config.MASTER_API_BASE_URL}${rewrittenPath}${queryPart ? '?' + queryPart : ''}`;
        // Preparar headers para la solicitud a la API maestra
        const methodHasBody = ['POST', 'PUT', 'PATCH'].includes(String(req.method).toUpperCase());
        const headers = {
            'X-Api-Key': config_1.config.MASTER_API_KEY,
            'X-Forwarded-For': String(req.ip || ''),
            'X-Original-Api-Key': String(req.apiKey || ''),
        ...
        // Realizar la solicitud a la API maestra
        ...
        // Establecer código de estado y enviar respuesta
